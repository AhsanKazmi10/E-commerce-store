...
import Image from 'next/image' // Next.js Optimized Image Engine
import { motion, AnimatePresence } from 'framer-motion'

// H.B Enterprises ke Satin Printing Department ki specialized asset images
const satinSliderImages = [
  "/saitnhero1.png", // Flexographic machine running white satin roll
  "/satinhero2.png", // Close-up shot of multi-color printed brand labels
  "/satinhero3.png", // Ultrasonic cutting line setup
]

function SatinLabelsSliderHero() {
  const [currentIndex, setCurrentIndex] = useState(0)

  // Auto-slide logic (Every 5 seconds)
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentIndex((prev) => (prev + 1) % satinSliderImages.length)
    }, 5000)
    ...
    <section className='w-full relative h-[500px] md:h-[600px] lg:h-[650px] overflow-hidden flex items-center justify-center bg-slate-950'>
      ...
      <div className="absolute inset-0 z-0">
        <AnimatePresence mode='wait'>
          ...
          >
            <Image
              src={satinSliderImages[currentIndex]}
              alt="Satin Production Background"
              fill
              priority
              sizes="100vw"
      ...
      <div className='container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full'>
        <div className='max-w-4xl mx-auto flex flex-col items-center text-center'>
          ...
          {/* Breadcrumb Navigation - Dark Blue Theme (Exactly like Patches) */}
          <motion.div 
          ...
          >
            ...
            <span className='text-blue-500 font-semibold'>Satin Labels</span>
          </motion.div>

          {/* Premium Headline (Exactly like Patches sizing and typography) */}
          <motion.h1 
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ delay: 0.5, duration: 0.8 }}
            className='text-4xl sm:text-5xl md:text-5xl lg:text-6xl font-black italic text-white leading-[1.1] tracking-tight uppercase mb-5'
          >
            PRINTED <br />
          ...
          >
            High-speed flexographic substrate line. Silk texture ribbon branding built for elite fashion apparel. Engineered for ultimate structural detail and heavy-duty durability.
          </motion.p>
          ...
          >
            <button 
              onClick={() => {
                const element = document.getElementById('satinShowcase'); // Make sure your lower section has this ID
                if (element) {
...
export default SatinLabelsSliderHero